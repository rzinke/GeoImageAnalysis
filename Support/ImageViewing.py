...
def plot_histogram(img, mask=None, bins=128, fig=None, ax=None):
...
def plot_smooth_histogram(img, mask=None, pts=1000, fig=None, ax=None):
    ''' Plot the histogram generated by image_histogram as a smooth plot. '''
    # Downsample image
    maxValues = 1E4
    skips = np.ceil(np.multiply(*img.shape)/maxValues).astype(int)
    dsampImg = img.flatten()[::skips]

    # Range of values
    valueRng = np.linspace(dsampImg.min(), dsampImg.max(), pts)
