...
### RASTER CROPPING ---
def crop_to_cutline(DS, cutlineName, verbose=False):
    ''' Crop the given data set to the cutline file provided. '''
    if verbose == True: print('Cropping to cutline: {:s}'.format(cutlineName))

    # Crop data set
    DS = gdal.Warp('', DS, format='MEM', cutlineDSName=cutlineName)
...
def sample_points_from_raster(img, x, y, mask=None, searchR=0, verbose=False):
    ...
    if verbose == True:
        ...
        print('\tusing search radius of {:d} pixels'.format(searchR*2-1))
