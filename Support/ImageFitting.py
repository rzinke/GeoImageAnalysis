...
def dates_to_datetimes(dates, datestr='%Y%m%d', verbose=False):
    ...
    if verbose == True:
        print('Converting {:d} dates to datetimes with format {:s}'.\
            format(len(dates), datestr))
...
### TIMESERIES FITTING ---
def fit_linear(x, y, degree=1, verbose=False, plot=False):
    ...
    if verbose == True:
        B = B.flatten()
        print('Linear fit')
        [print('\tx^{:d}: {:.4f}'.format(i, B[i])) for i in range(degree+1)]
...
def fit_surface_to_points_weighted(x, y, z, weights, degree=1, verbose=False):
    ...
    if verbose == True:
        ...
        print('Order: {:d}'.format(degree))
        print('\toffset: {:f}'.format(B[0]))
        [print('\tX^{i:d} {X:f} Y^{i:d} {Y:f}'.\
            format(**{'i':i, 'X':B[2*i-1], 'Y':B[2*i]})) \
            for i in range(1, degree+1)]
        print('RMS residual: {:f}'.format(rms))
...
def fit_surface_to_image(img, mask, degree=1, dx=1, dy=1, decimation=0,
        verbose=False):
    '''
    ...
    # Design matrix - depends on polynomial degree
    G = design_matrix2d(X.flatten(), Y.flatten(),
