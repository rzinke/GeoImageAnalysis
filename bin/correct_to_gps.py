...
class GPScorrection:
    ...
    def __compute_residuals__(self, searchRadius):
        '''
        Compute residuals (GPS - adjusted data).
        '''
        # Compute residuals
        _, _, self.sAdjusted, _ = sample_points_from_raster(self.imgAdjusted, self.sX, self.sY,
            mask=self.mask, searchR=searchRadius, verbose=self.verbose)
        self.resids = self.GPS.data - self.sAdjusted
    ...
    def plot(self):
        ...
        ## Plot adjusted image samples vs GPS
        # Spawn figure and axis
        FigCompare, axCompare = plt.subplots()

        # Plot points
        axCompare.scatter(self.GPS.data, self.sAdjusted, 36)

        # Plot 1:1 line
        GPSmin = self.GPS.data.min()
        GPSmax = self.GPS.data.max()
        axCompare.plot([GPSmin, GPSmax], [GPSmin, GPSmax], 'k--')

        # Format plot
        axCompare.set_xlabel('GPS points')
        axCompare.set_ylabel('Adjusted image points')
        axCompare.set_title('Adjusted image samples vs GPS')
