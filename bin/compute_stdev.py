...
'''
SHORT DESCRIPTION
Compute the image standard deviation.
...
from SpatialStatistics import imgstdev
from ImageViewing import plot_raster


### PARSER ---
Description = '''Compute the pixel-wise standard deviation of an image.'''
...
def createParser():
    ...
    InputArgs.add_argument('-w','--kernel-width', dest='kernelWidth', type=int, default=3,
        help='Kernel width. ([3], 5, ...)')
    ...
    OutputArgs.add_argument('-p','--plot', dest='plot', action='store_true',
        help='Plot results.')
    OutputArgs.add_argument('-o','--outname', dest='outName', type=str, default='StDev',
        help='Standard deviation map name.')
...
if __name__ == '__main__':
    ...
    SD = imgstdev(img, w=inps.kernelWidth, verbose=inps.verbose)
    ...
    save_gdal_dataset(outName, SD, mask=mask, exDS=DS, verbose=inps.verbose)
    ...
    if inps.plot == True:
        ...
        figSD, axSD = plot_raster(SD, mask=mask, extent=extent, cbarOrient='auto')
        axSD.set_title('Image standard deviation')
